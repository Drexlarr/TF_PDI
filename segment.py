...
# Hallar el mayor contorno
    ...
    if biggest.size != 0:
        ...
        imgAdaptiveThre, f = cv2.threshold(
            imgWarpGray, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)

        cv2.imshow("warpgray", f)

    else:
        cv2.imshow("warpgray", imgContours)

    cv2.imshow("image", imgContours)
    cv2.imshow("dilerode", ero)

    cv2.waitKey(300)
cv2.imwrite('./titanic_foto', f)
